Visioncoin Core version 0.13.2 is now available from:

  <https://download.visioncoin.org/visioncoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/codingbam/visioncoin/issues>
...
updates will be released anymore. Without security updates, using a visioncoin
...
ECDSA signatures inside Visioncoin transactions now use validation using
...
Previous versions of Visioncoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Visioncoin Core was to raise the default minimum
relay fee.

Visioncoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Visioncoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running visioncoind without having to do any manual configuration.
...
Visioncoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Visioncoin Core will relay transactions with
...
entirely in Visioncoin Core 0.13, and it is at this time undecided whether the
...
Visioncoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Visioncoin Core automatically creates a hidden service to listen on, without
manual configuration. Visioncoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Visioncoin Core is listening, and
...
Visioncoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Visioncoin
Core to use floating fees. By default, Visioncoin Core will use floating
fees.
...
At all times, Visioncoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) LTC.
Furthermore, Visioncoin Core will never create transactions paying less than
...
`visioncoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
Then, to tunnel a SSL connection on 27332 to a RPC server bound on localhost on port 19334 do:
    ...
    stunnel -d 27332 -r 127.0.0.1:17334 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /visioncoinrpc>
        ...
        # optional bypass visioncoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< visioncoinrpc:<password>
    </Location>
...
Restarting visioncoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in visioncoin.conf
...
visioncoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/visioncoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Visioncoin Core code base has
...
- `visioncoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `visioncoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Visioncoin scripting system that in combination with
...
Visioncoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Visioncoin Core.
...
The two phases are distinct in the Visioncoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Visioncoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Visioncoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Visioncoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of visioncoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of visioncoins being spent is exactly
...
- **Increased security for multisig:** Visioncoin addresses (both P2PKH addresses
  ...
  believed to be provided by Visioncoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Visioncoin's current ledger (called the UTXO set),
  ...
  note that Visioncoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Visioncoin users to individually opt-in to almost any change in the Visioncoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Visioncoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Visioncoin
client to Visioncoin Core 0.13.2 to signal for segwit support. When a
...
Since Visioncoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `visioncoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `visioncoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
